...
NSString *const STOPACTIVITYINDICATOR = @"STOPACTIVITYINDICATOR";
...
- (void)loadFailedWithError:(NSError *)error
{
	...
	_loading = NO;
	_cancelled = NO;
    [[NSNotificationCenter defaultCenter] postNotificationName:AsyncImageLoadDidFail object:_target userInfo:[NSDictionary dictionaryWithObjectsAndKeys: _URL, AsyncImageURLKey, error, AsyncImageErrorKey, nil]];
}
...
- (void)cacheImage:(UIImage *)image
{
	if (!_cancelled)
	{
        if (image && _URL)
        {
            ...
            if (storeInCache)
            {
				[[[AsyncImageLoader sharedLoader]dictOfImage] setObject:image forKey:_URL];
...
+ (AsyncImageLoader *)sharedLoader
{
	...
	if (sharedInstance == nil)
	{
		...
		sharedInstance.dictOfImage=[[NSMutableDictionary alloc]init];
	}
...
- (AsyncImageLoader *)init
{
	if ((self = [super init]))
	{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(targetReleased:) name:AsyncImageTargetReleased object:nil];
	}
	return self;
}
- (void)didReceiveMemoryWarning
...
- (void)dealloc
{
	...
	if(self.dictOfImage) self.dictOfImage=nil;
    [super ah_dealloc];
}
...
@implementation UIImageView(AsyncImageViewNew)

- (void)setImageURL:(NSURL *)imageURL withDictionary:(NSMutableDictionary *)dict
{
	[AsyncImageLoader sharedLoader].dictOfImage=dict;
...
@implementation AsyncImageViewNew
...
- (void)setImageURL:(NSURL *)imageURL withDictionary:(NSMutableDictionary *)dict
{
	...
	[super setImageURL:imageURL withDictionary:dict];
...
//	if (_showActivityIndicator && !self.image && imageURL)
    {
        if (_activityView == nil)
        {
			...
			[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(stopActivityIndicatorForImage) name:STOPACTIVITYINDICATOR object:nil];
        }
        [_activityView startAnimating];
    }
}
-(void)stopActivityIndicatorForImage
{
	if(_activityView)
	{
...
- (void)setImage:(UIImage *)image
{
	...
	super.image=[UIImage imageNamed:@"no_image_available.png"];
